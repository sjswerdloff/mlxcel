...
fn default_score_type() -> String { "max".to_string() }
fn default_local_block() -> usize { 1 }

impl ModelArgs {
    pub fn group_size(&self) -> i32 { 64 }
    pub fn bits(&self) -> i32 { 4 }
    pub fn gate_bits(&self) -> i32 { 8 }
    pub fn use_msa_for_layer(&self, layer_idx: usize) -> bool {
        let cfg = &self.text_config.sparse_attention_config;
        if !cfg.use_sparse_attention { return false; }
...
impl SparseAttention {
    pub fn forward(
        ...
        let q = if let Some(ref n) = self.q_norm { n.forward(&q) } else { q };
        let k = if let Some(ref n) = self.k_norm { n.forward(&k) } else { k };
        ...
        let idx_q = if let Some(ref n) = self.index_q_norm { n.forward(&idx_q_raw) } else { idx_q_raw };
        let idx_k = if let Some(ref n) = self.index_k_norm { n.forward(&idx_k_raw) } else { idx_k_raw };
        ...
        let idx_q = mlxcel_core::fast_rope(&idx_q, self.rope_dims, false, self.rope_base, 1.0, offset);
        let idx_k = mlxcel_core::fast_rope(&idx_k, self.rope_dims, false, self.rope_base, 1.0, offset);

        // Block max-pool scoring
        let k_blocked = mlxcel_core::reshape(
            &idx_k, &[b, 1, num_blocks, self.block_size, self.index_dim],
        );
        let k_pool = mlxcel_core::max_axis(&k_blocked, 3, false);

        let q_blocked = mlxcel_core::reshape(
            &idx_q, &[b, self.num_kv_heads, num_blocks, self.block_size, self.index_dim],
        );
    ...
    fn apply_causal_block_mask(&self, scores: &MlxArray, num_blocks: i32) -> UniquePtr<MlxArray> {
        ...
        let neg_inf = mlxcel_core::full_f32(&[1], f32::NEG_INFINITY, mlxcel_core::array_dtype(scores));
    ...
    fn sparse_sdpa(
        ...
        let k_blocked = mlxcel_core::reshape(
            k, &[b, self.num_kv_heads, num_blocks, self.block_size, self.head_dim],
        );
        let v_blocked = mlxcel_core::reshape(
            v, &[b, self.num_kv_heads, num_blocks, self.block_size, self.head_dim],
        );

        let indices_flat = mlxcel_core::reshape(
            selected, &[b * self.num_kv_heads * num_blocks * self.top_k],
        );
        ...
        let k_flat = mlxcel_core::reshape(
            &k_gathered, &[b, self.num_kv_heads, num_blocks, kv_len, self.head_dim],
        );
        let v_flat = mlxcel_core::reshape(
            &v_gathered, &[b, self.num_kv_heads, num_blocks, kv_len, self.head_dim],
        );

        let q_blocked = mlxcel_core::reshape(
            q, &[b, self.num_heads, num_blocks, self.block_size, self.head_dim],
        );
        ...
        let scores = mlxcel_core::matmul(
            &q_blocked, &mlxcel_core::transpose_axes(&k_expanded, &[0, 1, 2, 4, 3]),
        );
    ...
    fn dense_attention(
        &self, q: &MlxArray, k: &MlxArray, v: &MlxArray, mask: Option<&MlxArray>,
    ) -> UniquePtr<MlxArray> {
    ...
    pub fn from_weights(
        weights: &WeightMap, args: &ModelArgs, prefix: &str, has_index: bool,
    ) -> Result<Self, String> {
        ...
        let (q_norm, k_norm) = if cfg.use_qk_norm {
            ...
            (Some(RMSNorm::new(qw, cfg.rms_norm_eps)), Some(RMSNorm::new(kw, cfg.rms_norm_eps)))
        } else {
            (None, None)
        };

        let (index_q_proj, index_k_proj, index_q_norm, index_k_norm) = if has_index {
            let iqp = UnifiedLinear::from_weights(weights, &format!("{}.index_q_proj", prefix), g, b)?;
            let ikp = UnifiedLinear::from_weights(weights, &format!("{}.index_k_proj", prefix), g, b)?;
            let iqn = get_weight(weights, &format!("{}.index_q_norm.weight", prefix))
                .ok().map(|w| RMSNorm::new(w, cfg.rms_norm_eps));
...
impl SharedExperts {
    pub fn forward(&self, x: &MlxArray, alpha: f32, limit: f32) -> UniquePtr<MlxArray> {
        swiglu_forward(x, &self.gate_proj, &self.up_proj, &self.down_proj, alpha, limit)
    }

    pub fn from_weights(weights: &WeightMap, prefix: &str, g: i32, b: i32) -> Result<Self, String> {
        Ok(Self {
            gate_proj: UnifiedLinear::from_weights(weights, &format!("{}.gate_proj", prefix), g, b)?,
            up_proj: UnifiedLinear::from_weights(weights, &format!("{}.up_proj", prefix), g, b)?,
            down_proj: UnifiedLinear::from_weights(weights, &format!("{}.down_proj", prefix), g, b)?,
        })
    }
}
...
impl SparseMoeBlock {
    pub fn forward(&self, x: &MlxArray) -> UniquePtr<MlxArray> {
        ...
        let mut result = crate::models::switch_layers::moe_weighted_sum(
            &expert_out, &norm_scores, mlxcel_core::array_dtype(&x_flat),
        );
    ...
    pub fn from_weights(weights: &WeightMap, args: &ModelArgs, prefix: &str) -> Result<Self, String> {
        let g = args.group_size();
        let b = args.bits();
        let cfg = &args.text_config;

        let router = UnifiedLinear::from_weights(weights, &format!("{}.gate", prefix), g, args.gate_bits())?;

        let experts = SwitchGLU::from_weights_with_proj_names(
            weights, &format!("{}.switch_mlp", prefix), g, b, ["w1", "w3", "w2"],
        )?;

        let shared = if cfg.n_shared_experts > 0 {
            Some(SharedExperts::from_weights(weights, &format!("{}.shared_experts", prefix), g, b)?)
        } else {
            None
        };

        let bias_key = format!("{}.e_score_correction_bias", prefix);
        let e_score_correction_bias = weights.get(&bias_key)
            .map(|w| mlxcel_core::copy(w))
            .unwrap_or_else(|| mlxcel_core::full_f32(&[cfg.num_local_experts as i32], 0.0, mlxcel_core::dtype::FLOAT32));

        Ok(Self {
            router, experts, shared_experts: shared, e_score_correction_bias,
...
impl DenseMLP {
    pub fn forward(&self, x: &MlxArray) -> UniquePtr<MlxArray> {
        swiglu_forward(x, &self.gate_proj, &self.up_proj, &self.down_proj, self.swiglu_alpha, self.swiglu_limit)
    }

    pub fn from_weights(weights: &WeightMap, prefix: &str, g: i32, b: i32, alpha: f32, limit: f32) -> Result<Self, String> {
        Ok(Self {
            gate_proj: UnifiedLinear::from_weights(weights, &format!("{}.gate_proj", prefix), g, b)?,
            up_proj: UnifiedLinear::from_weights(weights, &format!("{}.up_proj", prefix), g, b)?,
            down_proj: UnifiedLinear::from_weights(weights, &format!("{}.down_proj", prefix), g, b)?,
...
impl DecoderLayer {
    pub fn forward(&self, x: &MlxArray, cache: &mut KVCache, mask: Option<&MlxArray>) -> UniquePtr<MlxArray> {
    ...
    pub fn from_weights(weights: &WeightMap, args: &ModelArgs, layer_idx: usize) -> Result<Self, String> {
        ...
        let (mlp, moe) = if is_moe {
            ...
            (None, Some(SparseMoeBlock::from_weights(weights, args, &moe_prefix)?))
        } else {
            let mlp_prefix = format!("{}.mlp", prefix);
            let mlp = DenseMLP::from_weights(
                weights, &mlp_prefix, args.group_size(), args.bits(),
                args.text_config.swiglu_alpha, args.text_config.swiglu_limit,
            )?;
            (Some(mlp), None)
        };

        let input_norm = get_weight(weights, &format!("{}.input_layernorm.weight", prefix))?;
        let post_norm = get_weight(weights, &format!("{}.post_attention_layernorm.weight", prefix))?;
        ...
        Ok(Self { self_attn, mlp, moe, input_layernorm, post_attention_layernorm })
    }
}
...
impl MiniMaxM3Model {
    pub fn forward(&self, input_ids: &MlxArray, caches: &mut [KVCache], mask: Option<&MlxArray>) -> UniquePtr<MlxArray> {
    ...
    pub fn load<P: AsRef<Path>>(model_dir: P) -> Result<(Self, ModelArgs), String> {
        ...
        let text_config = full_config.get("text_config").ok_or("Missing text_config")?;
    ...
    pub fn from_weights_with_prefix(weights: &WeightMap, args: &ModelArgs, prefix: &str) -> Result<Self, String> {
        let g = args.group_size();
        let b = args.bits();
        let embed_tokens = UnifiedEmbedding::from_weights(weights, &format!("{}.embed_tokens", prefix), g, b)?;
        ...
        let lm_head = if !args.text_config.tie_word_embeddings {
            Some(UnifiedLinear::from_weights(weights, "language_model.lm_head", g, b)?)
        } else {
            None
        };
...
impl LanguageModel for MiniMaxM3Model {
    fn forward(&self, input_ids: &MlxArray, caches: &mut [KVCache], mask: Option<&MlxArray>) -> UniquePtr<MlxArray> {
        MiniMaxM3Model::forward(self, input_ids, caches, mask)
    }
    fn make_caches(&self) -> Vec<KVCache> { MiniMaxM3Model::make_caches(self) }
    fn num_layers(&self) -> usize { self.layers.len() }
    fn eos_token_ids(&self) -> Vec<i32> { vec![200020] }
}
...
mod tests {
    ...
    fn test_m3_config_parsing() {
        ...
        assert_eq!(args.text_config.sparse_attention_config.sparse_block_size, 128);
        assert_eq!(args.text_config.sparse_attention_config.sparse_topk_blocks, 16);
    ...
    fn test_swiglu_oai_matches_python() {
        let alpha = 1.702f32;
        let limit = 7.0f32;
        let test_cases = vec![(0.0f32, 0.0f32), (1.0f32, 1.0f32), (-1.0f32, -1.0f32), (10.0f32, 10.0f32), (-10.0f32, -10.0f32)];
        ...
        for (gate_val, up_val) in test_cases {
            ...
            assert!(
                ...
                "SwiGLU mismatch gate={} up={}: rust={} py={}", gate_val, up_val, rust_result, py_result
            );
        }
    }
    ...
    fn test_causal_block_mask() {
        ...
        for q in 0..num_blocks {
            for k in 0..num_blocks {
                if k > q { assert!(k > q, "Block {} should NOT see block {}", q, k); }
                else { assert!(k <= q, "Block {} should see block {}", q, k); }
            }
        }
    }
    ...
    fn test_topk_selection() {
        let scores = vec![0.1, 0.9, 0.3, 0.8, 0.2, 0.7, 0.4, 0.6];
        let k = 3;
