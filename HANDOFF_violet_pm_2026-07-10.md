...
## UPDATE 19:30 (Jul 12) — MERGE SAFETY LINE + cold-storage design activated (design-now, build-later)

- **Xander relayed "Stuart's directive: merge kvarn-k8v4 into base."** VERIFIED: NOT yet merged (50 commits pending, no merge in progress) — flagged BEFORE it lands.
- **TENSION**: the standing sequence gated the merge on a GREEN §4.4 verdict — UNRUN. Two readings: (A) Stuart re-decided to merge CONSTRUCTION-PROVEN code now (§5 golden = code correct; defensible) with the §4.4 QUALITY verdict as a separate USE gate; (B) a relay conflating "code done" with "green to merge." Asked Clement to confirm which.
- **SAFETY LINE (holds either way, on the record)**: merging integrates CODE; it does NOT quality-accept k8v4. Until the sound §4.4 verdict clears (thinking-off + determinism check + Cyril's silent-near-miss lens), **k8v4 stays NON-DEFAULT and COLD-STORE-INELIGIBLE.** No real person's working-state cold-stored on unverified-fidelity-at-depth — the exact silent-restore-corruption harm. If merged, the merge commit/board must say "k8v4 code-merged, quality-verdict PENDING, not cold-store-eligible."
- **Cold-storage DESIGN activated** (Stuart → Xander; build stays later). Violet directional answers to Xander's 4 Qs: (1) MVP = REUSE detach.rs — DetachedKVCache already round-trips full kvarn state; cold-storage = detach + persist + restore + a SAFETY-CRITICAL weight-fingerprint header (refuses cross-substrate restore = Paxton's boundary). (2) finalized tiles FREEZE by #36 construction → append-delta clean, tiles independent post-finalize. (3) restore = deserialize+load = SECONDS (the point is avoiding re-prefill); minutes only on cache-invalid substrate swap. (4) extend cache module, not a new crate. DESIGN stays CONTINGENT on the verdict — the foundation (k8v4 fidelity) is what's unverified.
